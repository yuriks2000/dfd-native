...
// INSERT POINT: _translate
	...
	_initEvents: function (remove) {
		...
		if ( this.options.click ) {
			eventType(this.wrapper, 'click touchend', this, true);
		}
...
(function(global, factory) {
    ...
    // slide
    var SLIDE_DEFAULT_SEL =     '.dfd-onepage-slide';
    var SLIDE =                 'fp-slide';
    var SLIDE_SEL =             '.' + SLIDE;
    ...
    $.fn.fullpage = function(options) {
		...
		/* Added by DFD. 
		 * 
		 * Returns the available eight of the container (window height without admin bar, frame border and header)
		 */
		function getAvailHeight() {
			var availHeight = $(window).height(),
				offset = 0,
				$header = $('#header-container'),
				scrollbarWidth = typeof dfd_native != 'undefined' && typeof dfd_native.scrollbarWidth != 'undefined' ? dfd_native.scrollbarWidth : 0;
			
			if($('#wpadminbar').length > 0 && !$('body').hasClass('compose-mode')) {
				offset += $('#wpadminbar').outerHeight();
			}
			
			if($('.dfd-frame-line.line-top').length > 0) {
				offset += $('.dfd-frame-line.line-top').outerHeight() * 2;
			}
			
			if($(window).width() + scrollbarWidth < 1101) {
				...
				if(
					...
					offset += ($header.find('.dfd-top-row').length > 0) ? $header.find('.dfd-top-row').outerHeight() : $header.outerHeight();
				}
			}
        ...
        */
        function moveSectionUp(){
			...
			if($('body').hasClass('dfd-one-page-footer-expanded')) {
				setTimeout(function() {
					$('body').removeClass('dfd-one-page-footer-expanded');
				}, 600);
				return false;
			}
        ...
        */
        function moveSectionDown(){
			...
			if(!next.length && $('#footer-wrap').length > 0 && !$('body').hasClass('dfd-one-page-footer-expanded')) {
				$('body').addClass('dfd-one-page-footer-expanded');
			}
        ...
        */
        ...
        function reBuild(resizing){
            if(container.hasClass(DESTROYED) || !canScroll){ return; }  //nothing to do if the plugin was destroyed

            isResizing = true;

            windowsHeight = getAvailHeight();  //updating global var
			
			if(container.hasClass('dfd-enable-onepage-animation') || container.hasClass('dfd-disable-onepage-animation')) {
        ...
        */
        function scrollPage(element, callback, isMovementUp){
            if(typeof element === 'undefined' || !canScroll){ return; } //there's no element to scroll, leaving the function
		...
		var dfdAnimations = {
			...
			init: function(options, style) {
				var animation = 'dfdAnimation'+style;
				if(options) {
			...
			animate: function() {
				var self = this,
					speed = originals.scrollingSpeed - 100;
        ...
        */
        function performMovement(v){
			var animatioEffect = container.hasClass('dfd-enable-onepage-animation');
			...
			if(animatioEffect) {
				...
				if(anim.indexOf("dfd-3d-style-") != -1) {
					anim = anim.substring(anim.toLowerCase().indexOf("dfd-3d-style-"));
					anim = anim.substring(13, 14);
				}
				if(typeof dfdAnimations != 'undefined' && anim.length == 1) {
					dfdAnimations.init(v, anim);
				}
			}
        }
...
//                    $(this).get(0).contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}','*');
        ...
        */
        function createScrollBar(element){
			...
			if(
				...
				headerHeight = ($header.find('.dfd-top-row').length > 0) ? $header.find('.dfd-top-row').outerHeight() : $header.outerHeight();
			}
        ...
        function addTableClass(element){
            ...
            if(!element.hasClass(TABLE)){
				if($('body').hasClass('compose-mode')) {
					element.addClass(TABLE).find('> .vc-row-wrapper').wrapInner('<div class="' + TABLE_CELL + '" style="height:' + getTableHeight(element) + 'px;" />');
				} else {
        ...
        */
        function scrollPageAndSlide(destiny, slide){
            ...
            if(!section.length || !canScroll) return;
...
(function($) {
	"use strict";
	var $container = $('#layout.one-page-scroll'),
		$headerContainer = $('#header-container'),
		smartHeader = false,
		lightLogo = false,
		darkLogo = false,
		dots = true,
		dfdInitOnePage = function() {
			$container.fullpage({
				navigation: dots,
				lazyLoading: true,
				scrollBar: false,
	//			//scrolling
				...
				afterLoad: function(anchorLink, index) {
					if(smartHeader && $headerContainer.hasClass('dfd-header-layout-fixed') && !$headerContainer.hasClass('header-style-12') && !$headerContainer.hasClass('header-style-13') && !$headerContainer.hasClass('header-style-14')) {
						if($('body').hasClass('compose-mode')) {
							var $el = $container.find('> div.vc_vc_row > .vc-row-wrapper:nth-child('+index+')');
						} else {
							$el = $container.find('> .vc-row-wrapper:nth-child('+index+')')
						}
						if($el.hasClass('dfd-background-dark')) {
							...
							if(lightLogo && lightLogo != '') {
								if($headerContainer.hasClass('header-style-8') || $headerContainer.hasClass('header-style-9')) {
									$headerContainer.find('.dfd-top-row .main-logo').attr('src', lightLogo);
								} else {
									$headerContainer.find('.main-logo').attr('src', lightLogo);
								}
							}
						} else {
							...
							if(darkLogo && darkLogo != '') {
								if($headerContainer.hasClass('header-style-8') || $headerContainer.hasClass('header-style-9')) {
									$headerContainer.find('.dfd-top-row .main-logo').attr('src', darkLogo);
								} else {
									$headerContainer.find('.main-logo').attr('src', darkLogo);
								}
							}
						}
					}
	...
	if($container.length > 0) {
		if($headerContainer.length > 0 && $container.data('smart-header') && $container.data('smart-header') == '1') {
			$headerContainer.addClass('dfd-smart-header');
			smartHeader = true;
			if($container.data('light-logo')) {
				lightLogo = $container.data('light-logo');
			}
			if($container.data('dark-logo')) {
				darkLogo = $container.data('dark-logo');
			}
		}

		if($('#stuning-header').length > 0) {
			$('#stuning-header').remove();
		}

		$container.find('> div.vc-row-wrapper').addClass('section');
		...
		$(window).load(function() {
			if($('body').hasClass('compose-mode')) {
				$container.find('> div.vc_vc_row').addClass('section');
				$.fn.fullpage.destroy('all');
				dfdInitOnePage();
				$('body').on('post-load', function() {
					if($container.find('> div.vc_vc_row:not(.section)').length > 0) {
						$container.find('> div.vc_vc_row:not(.section)').addClass('section');
						$.fn.fullpage.destroy('all');
						dfdInitOnePage();
						$('body').trigger('vc-front-reinited');
					}
				});
			}
		});
