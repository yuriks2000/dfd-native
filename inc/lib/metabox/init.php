...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		$dir = get_template_directory().'/inc/lib/metabox';
	...
	public function register_scripts() {
		...
		global $wp_version, $dfd_native;
		// Only use minified files if SCRIPT_DEBUG is off
		$min = (isset($dfd_native['dev_mode']) && $dfd_native['dev_mode'] == 'on' && defined('DFD_DEBUG_MODE') && DFD_DEBUG_MODE) ? '' : '.min';
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => esc_html__( 'Clear' ,'dfd-native' ),
					'defaultString' => esc_html__( 'Default', 'dfd-native' ),
					'pick'          => esc_html__( 'Select Color', 'dfd-native' ),
					'current'       => esc_html__( 'Current Color', 'dfd-native' ),
				) );
			}
		} else {
...
//		wp_register_script( 'cmb-scripts', CMB_META_BOX_URL .'js/cmb.js', $scripts, self::CMB_VERSION );
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => esc_html__( 'Select / Deselect All', 'dfd-native' ),
		) ) );

		//wp_register_style( 'cmb-styles', CMB_META_BOX_URL . '/stylesheets/screen.css', $styles );
	...
	public static function render_group( $args ) {
		...
		$data_option = isset($args['dep_option']) && !empty($args['dep_option']) ? $args['dep_option'] : false;
		$data_val = isset($args['dep_values']) && !empty($args['dep_values']) ? $args['dep_values'] : false;
		
		if($data_option) {
			$data_option = 'data-dependancy-option="'.esc_attr($data_option).'"';
		}
		
		if($data_val) {
			$data_val = 'data-dependancy-value="'.esc_attr($data_val).'"';
		}

		echo '<tr '.$data_option.' '.$data_val.'><td class="dfd-metabox-repeater" colspan="2"><table id="', $field_group->id(), '_repeat" class="repeatable-group'. $sortable .'" style="width:100%;">';
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
		...
		echo '<tr>'
				. '<td>'
					. '<p class="add-row">'
						. '<button data-selector="', $field_group->id() ,'_repeat" data-grouptitle="', $field_group->options( 'group_title' ) ,'" class="add-group-row button">'. $field_group->options( 'add_button' ) .'</button>'
					. '</p>'
				. '</td>'
			. '</tr>';
	...
	public static function save_field( $new_value, $field ) {
		...
		// 	}
		// } else
		if ( (! empty( $new_value ) || 0 == $new_value) && $new_value != $old  ) {
//		if ( $new_value != '' && $new_value != $old  ) {
			self::$updated[] = $name;
			return $field->update_data( $new_value );
		} elseif ( empty( $new_value ) && 0 != $new_value ) {
//		} elseif ( $new_value == '' ) {
			if ( ! empty( $old ) )
				self::$updated[] = $name;
	...
	public static function get_meta_box_url() {
		$cmb_url = get_template_directory_uri().'/inc/lib/metabox';
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	if ( $echo ) {
		echo sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'dfd-native' ) );;
	}

	return sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'dfd-native' ) );;
}
